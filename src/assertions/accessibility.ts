...
interface A11yRule {
  id: string;
  description: string;
  check: (html: string) => { passed: boolean; nodes: string[] };
}
...
const RULES: A11yRule[] = [
  ...
  },
  {
    ...
    check: (html) => {
      ...
      if (!hasHtml) return { passed: true, nodes: [] };
...
export function checkAccessibility(
  ...
  const violations: A11yCheckResult['violations'] = [];
  let passCount = 0;

  for (const rule of RULES) {
    if (ignoreRules.includes(rule.id)) continue;
